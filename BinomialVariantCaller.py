...
"""
Usage:
    BinomialVariantCaller.py (--pileup=STR) (--p=STR) [-o STR]
...
def VCFHeader(sample):
    ...
    VCFheader.add_line("##FORMAT=<ID=VAF,Number=1,Type=String,Description=Variant allele frequency>")
...
for line in pileup_file:
    ...
    if genotype != (0,0):
        vaf = round(polymorphic_site[0][1]/rec.rCount,2)
        alts = DetermineAltsField(polymorphic_site)
        VCF_line = VCFBodyLine(rec, alts, genotype, vaf, VCF_out, sampleID)
